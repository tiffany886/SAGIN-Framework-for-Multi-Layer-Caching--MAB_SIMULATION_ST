...
class UAV:
    """
    UAV类（无人机类），用于模拟SAGIN（Space-Air-Ground Integrated Network）框架中的无人机节点。
    UAV负责内容缓存、请求处理、多臂老虎机（MAB）算法执行，以及与其他UAV、卫星和地面站的通信。
    支持多种缓存算法，包括LRU、Popularity、MAB_Original、MAB_Contextual、Federated_MAB等。
    """
    def __init__(self, uav_id, grid_size, uav_grid_size, aggregator, algorithm="MAB_Contextual"):
        ...
        参数:
        uav_id (str): UAV的唯一标识符，如"UAV1"、"UAV2"等。
        grid_size (int): 整个网络的网格大小。
        uav_grid_size (int): 单个UAV覆盖的网格大小。
        aggregator (object): 联邦聚合器，用于MAB算法的协调。
        algorithm (str): 使用的缓存算法，默认"MAB_Contextual"。

        该方法设置UAV的基本属性、覆盖区域、缓存空间、学习组件和性能跟踪器。
        """
        # 基础身份信息
        self.uav_id = uav_id # UAV唯一标识符
        self.grid_size = grid_size # 整个网络的网格大小
        self.uav_grid_size = uav_grid_size # 单个UAV覆盖的网格大小
        self.content_categories = ['II', 'III', 'IV'] # 无人机只生成 II, III, IV 类内容（图像、传感器、视频）
        self.content_size = {'I': 10, 'II': 1, 'III': 0.01, 'IV': 2} # 内容大小
        self.cache_size = 100 # 缓存容量 100 MB
        self.coverage_area = self.calculate_coverage_area() # 计算覆盖的网格范围
        self.transmission_rate = 50 # 传输速率 50 Mbps
        self.propagation_delay = 100  # 传播延迟（预留）
        self.content_cache = {} # 缓存的内容（统一的三级字典结构） 无人机当前缓存的所有内容
        self.epsilon = 0.1  # 探索概率
        self._period_seen_keys = set()  # 用于去重的临时集合

        # 算法选择
        self.algorithm = algorithm # 当前使用的缓存算法
        print(f"  📦 {self.uav_id} initialized with {self.algorithm} algorithm")

        # 动作空间和学习组件
        self.num_actions = 0 # 动作空间的大小
        self.action_space = [] # 候选内容列表，每次做缓存决策时，会把所有可能缓存的内容（包括自己的、邻居的、卫星的）都放进这个列表，然后算法从里面挑选。
        self.generated_cache = {'II': [], 'III': [], 'IV': []} #无人机自己生成的内容，按类别存放。
        self.record = {} # 学习记录，也是四层嵌套字典，记录每个内容被请求了多少次、命中了多少次、Q值是多少等。这是它的“学习笔记本”。

        # 性能跟踪
        self.num_context_parameters = 3                       # 上下文特征数量
        self.request_receive_count = 0                         # 总接收请求数
        self.request_receive_from_other_source = 0             # 从其他源收到的请求数（用于缓存查找）
        self.global_request_receive_count = 0                  # 全局请求计数
        self.content_hit_from_source = 0                       # 从源头直接命中的次数
        self.content_hit_from_indirect_source = 0              # 从间接源命中的次数
        self.current_slot = None                               # 当前时隙
        self.current_slot_content_requests = []                # 当前时隙收到的请求列表
        self.content_hit = 0                                    # 缓存命中总次数
        self.optimal_content_hit = 0                            # 理想命中次数（预留）
        self.avg_decision_latency = 0                           # 平均决策延迟
        self.decision_count = 0                                 # 决策次数
        self.aggregator = aggregator                           # 联邦聚合器

        # 邻居列表
        self.neighbors = []

        # 更细粒度的命中统计
        self.content_hit_for_direct_uav = 0                    # 直接从本机生成命中
        self.content_hit_for_sagin_links = 0                   # 通过无人机间链路命中
        self.content_hit_source_uav_1st_hop = 0                # 从本机源头命中（一跳）
        self.content_hit_source_uav_2nd_hop = 0                # 从邻居源头命中（两跳）
        self.content_hit_source_sat_2nd_hop = 0                # 从卫星源头命中（两跳）
        self.content_hit_cache_1st_hop = 0                     # 从本机缓存命中
        self.content_hit_cache_2nd_hop = 0                     # 从邻居缓存命中
        self.total_request_for_caching = 0                      # 需要缓存查找的请求总数
        self.total_request_received = 0                         # 接收到的请求总数
        # 初始化Q值为0
    ...
    def cache_cleanup(self, current_time):
        ...
        # 遍历三级字典: type -> coord -> category -> [contents]
                    ...
                    for content in cached_contents:
                        if isinstance(content, dict):  # Safety check
                            ...
                            if current_time < generation_time + validity:
                                valid_contents.append(content) # 未过期则保留
    ...
    def calculate_coverage_area(self):
        """
        鲁棒的覆盖区域计算方法，适用于任何网格配置。
        处理所有情况：uav_grid_size大于、等于或小于grid_size。

        该方法根据UAV的ID和网格参数计算其覆盖的区域范围，
        确保每个UAV都有合理的覆盖区域。
        """
        """
    ...
    def process_content_request(self, communication, requesting_vehicle, content_request, vehicles, uavs, satellites,
        ...
        """
        处理内容请求的核心方法。
        UAV根据请求的内容类型和来源，尝试从本地缓存、邻居缓存或卫星获取内容。

        参数:
        communication (Communication): 通信模块对象。
        requesting_vehicle (Vehicle): 请求内容的车辆对象。
        content_request (dict): 内容请求字典，包含类型、坐标、类别、编号等。
        vehicles (list): 所有车辆对象的列表。
        uavs (list): 所有UAV对象的列表。
        satellites (list): 所有卫星对象的列表。
        slot (int): 当前时间槽。
        communication_schedule (dict): 通信调度表。

        返回:
        content or None: 如果找到内容则返回内容对象，否则返回None。
        ...
        requested_entity_type = content_request['type'] # 请求的类型：satellite/UAV/grid
        requested_coord = content_request['coord']               # 源编号
        requested_category = content_request['category']         # 类别
        requested_content_no = content_request['no']             # 内容编号
        ...
        entry['request_tracking'] += 1                             # 记录请求次数
        self.global_request_receive_count += 1                     # 全局请求计数
                ...
                if requested_category in self.generated_cache:
                    ...
                    for content in category_cache:
                        if content['content_no'] == requested_content_no:
                            # 计算传输延迟：内容大小（MB）*8 得到 Mbit，除以传输速率（Mbps）得到秒，再加随机传播延迟
        ...
        if requested_entity_type in ("UAV", "satellite"):
            ...
            # neighbor cache 2-hop # 如果本地没有，查邻居的缓存
            for neighbor in self.neighbors:
    ...
    def track_content_hit(self, content, slot, observed_delay_ms=None, from_cache=False):
        ...
        if from_cache:
            ...
            self.cache_hits_tracked = getattr(self, 'cache_hits_tracked', 0) + 1  # ✅ correctly indented
    ...
    def select_action(self):
        """
        统一的算法分发器，根据当前算法路由到相应的缓存决策方法。
        支持多种缓存算法：LRU、Popularity、MAB_Original、MAB_Contextual、Federated_MAB、Enhanced_Federated_MAB。
        这是SAGIN框架中UAV缓存决策的核心入口点。
        """
        """
        ...
        if self.algorithm == "LRU":
            return self.select_action_lru() # 最近最少
        elif self.algorithm == "Popularity":
            return self.select_action_popularity() # 基于流行度
        elif self.algorithm == "MAB_Original":
            return self.select_action_mab_original() # 原始多臂老虎机
        elif self.algorithm == "MAB_Contextual":
            return self.select_action_mab_contextual() # 情境多臂老虎机
        elif self.algorithm == "Federated_MAB":
            return self.select_action_federated_mab() # 联邦多臂老虎机
        elif self.algorithm == "Enhanced_Federated_MAB":
            return self.select_action_enhanced_federated_mab() # 增强联邦
        else:
    ...
    def select_action_lru(self):
        """
        LRU（最近最少使用）缓存算法实现。
        根据内容的访问时间戳（slot）进行排序，优先缓存最近访问的内容。
        这是最简单的缓存替换策略，适用于UAV在SAGIN网络中的内容缓存决策。
        """
    ...
    def select_action_popularity(self):
        """
        基于流行度（Popularity）的缓存算法实现。
        根据内容的请求频率进行排序，优先缓存最受欢迎的内容。
        这种算法适用于内容访问模式相对稳定的场景，在SAGIN网络中可以提高缓存命中率。
        """
    ...
    def select_action_mab_original(self):
        """
        原始多臂老虎机（MAB）算法的缓存决策实现。
        使用UCB（Upper Confidence Bound）算法结合ε-贪婪策略进行内容选择。
        根据内容的Q值、奖励历史和探索因子来决定缓存哪些内容。
        这是SAGIN框架中UAV自主学习缓存策略的基础算法。
        """
        ...
        for action in self.action_space:
            ...
            # Tiny satellite prefetch bias that fades as it gets cached more
            try:
                ct = (action.get('content_type') if isinstance(action, dict)
                      else (action[0] if isinstance(action, tuple) and len(action) >= 1
                            else str(action).split('_', 1)[0]))
            except Exception:
                ct = ""
            if str(ct) == 'satellite':
                cached_times = int(record_entry.get('no_f_time_cached', 0))
                enhanced_q_value += float(getattr(self, 'sat_prefetch_boost', 0.35)) / (1.0 + cached_times)
    ...
    def select_action_mab_contextual(self):
        """
        上下文感知多臂老虎机（Contextual MAB）算法的缓存决策实现。
        结合内容的Q值和上下文特征（流行度、新鲜度、缓存效率）进行决策。
        通过上下文增强提高缓存决策的准确性，避免了原始MAB中探索过度的问题。
        适用于SAGIN网络中UAV需要考虑多种因素的复杂缓存场景。
        """
        """
    ...
    def debug_cache_hits(self, slot):
        ...
        if slot % 10 == 0:
            ...
            # request_receive_count 这个属性（总接收请求数）
            ...
            # 初始化两个计数器：total_hits 记录所有内容的命中次数总和，
            # content_with_hits 记录有多少个内容至少被命中过一次。
            total_hits = 0
            content_with_hits = 0
            # 类型 → 坐标 → 类别 → 内容编号 → 信息
            for entity_type, coord_dict in self.record.items():
                for coord, category_dict in coord_dict.items():
                    for category, content_no_dict in category_dict.items():
                        for content_no, content_info in content_no_dict.items():
                            hits = content_info.get('content_hit', 0) # content_hit 值（即该内容被缓存命中的次数）
                            if hits > 0: # 如果大于0，就累加到总命中数，增加有命中的内容计数，并打印具体哪个内容被命中多少次。
                                total_hits += hits 
                                content_with_hits += 1
    ...
    def select_action_federated_mab(self):
        """
        联邦多臂老虎机（Federated MAB）算法的缓存决策实现。
        通过聚合器（aggregator）与其他节点的Q值进行协作学习。
        将本地Q值与其他节点的Q值进行加权融合，提高缓存决策的准确性。
        这是SAGIN网络中实现分布式协作缓存的关键算法。
        """
        """
    ...
    def select_action_enhanced_federated_mab(self):
        """
        增强联邦多臂老虎机（Enhanced Federated MAB）算法的缓存决策实现。
        在基础联邦MAB基础上增加了性能权重和信任因子机制。
        通过考虑节点的性能表现和邻居节点的可靠性来调整协作权重。
        这是SAGIN网络中最先进的分布式缓存算法。
        """
        """
        ...
        for action in self.action_space:
            ...
            # Get aggregated neighbor Q-values
            neighbor_q_values = []
            if hasattr(self, 'aggregator') and self.aggregator:
                neighbor_values = self.aggregator.get_neighbor_q_values(
                    node_id,
            ...
            if neighbor_q_values:
                enhanced_score = 0.7 * local_q_value + 0.3 * np.mean(neighbor_values)
            else:
                enhanced_score = local_q_value

            action['enhanced_score'] = enhanced_score
    ...
    def _cache_content_helper(self, content):
        """
        统一的缓存内容辅助方法，适用于所有节点类型（BS、Vehicle、UAV）。
        创建标准的三层缓存结构：content_cache[type][coord][category] = [contents]。
        同时更新记录结构以跟踪缓存统计信息。
        这是SAGIN框架中缓存管理的核心辅助方法。
        """
        """
        ...
        content_type = content.get('content_type')  # 'satellite', 'UAV', 'terrestrial'  # 内容类型：satellite/UAV/grid
        content_coord = content.get('content_coord')  # Sat ID, UAV ID, or grid coord # 源编号
        # ALWAYS convert coord to string 始终转化成坐标字符串
    ...
    def clear_cache(self):
        """
        清空现有的内容缓存。
        在每次缓存决策周期开始时调用，确保缓存空间被完全重置。
        这是缓存管理的基本操作，在SAGIN框架的缓存更新周期中被频繁使用。
        """
    ...
    def get_reward(self):
        """
        统一的奖励分发器，根据当前算法路由到相应的奖励计算方法。
        支持多种缓存算法的奖励机制，包括LRU、Popularity、各种MAB变体等。
        这是SAGIN框架中UAV奖励机制的核心入口点，用于更新学习参数。
        """
        """
    ...
    def get_reward_lru(self):
        """
        LRU算法的奖励方法。
        LRU是基于规则的算法，不需要学习奖励机制，只跟踪最近使用情况。
        在SAGIN框架中，LRU作为基准算法，不涉及强化学习。
        """
    ...
    def get_reward_popularity(self):
        """
        基于流行度的奖励方法。
        跟踪内容的请求频率，用于Popularity-based缓存决策。
        计算每个内容的流行度得分，为后续的缓存决策提供依据。
        """
    ...
    def get_reward_mab_original(self):
        """
        原始多臂老虎机算法的奖励计算方法。
        基于缓存命中率和延迟性能计算奖励值，使用学习率更新Q值。
        这是SAGIN框架中UAV强化学习的基础奖励机制。
        """
    ...
    def get_reward_mab_contextual(self):
        """
        上下文感知多臂老虎机算法的奖励计算方法。
        在基础奖励基础上加入上下文特征的权重调整。
        结合流行度、新鲜度和缓存效率等上下文信息来优化奖励计算。
        这是SAGIN框架中考虑多维度因素的先进奖励机制。
        """
        lr = getattr(self, "learning_rate", 0.1)
        theta_ms_default = getattr(self, "theta_ms_default", 2000.0)
        cap = getattr(self, "delay_ratio_cap", 5.0)
    ...
    #  如果无人机使用的是联邦 MAB，它会在每个缓存周期结束时调用，把reward发送给聚合器
    ...
    def run(self, current_time, communication, communication_schedule, slot, satellites, no_of_content_each_category,
        ...
        # uav_content_generation_period时间槽生成一次新数据
        if (slot - 1) % uav_content_generation_period == 0:
            ...
            self.cache_cleanup(current_time)  # 清理过期内容
            if slot > 10: # 更新Q值
                self.get_reward()
            # 缓存决策
