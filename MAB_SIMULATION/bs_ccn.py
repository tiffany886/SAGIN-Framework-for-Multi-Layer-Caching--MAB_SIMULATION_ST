...
class BaseStation:
    """
    基站类（Base Station），用于模拟SAGIN框架中的基站行为。
    基站负责内容缓存、请求处理和多臂老虎机（MAB）算法的执行。
    支持多种缓存算法，包括LRU、Popularity、MAB_Original、MAB_Contextual等。
    """
    ...
    def select_action_mab_original(self):
        """
        使用原始MAB算法选择缓存动作。
        该方法实现基于UCB（Upper Confidence Bound）的多臂老虎机算法，
        结合延迟感知的奖励机制和流行度权重，用于决定哪些内容应该被缓存。
        包含详细的调试日志用于性能分析。
        """
        print(f"\n🏢 BS DEBUG - {self.bs_id}")
    ...
    def select_action_enhanced_federated_mab(self):
        """
        使用增强联邦MAB算法选择缓存动作。
        该方法结合本地Q值和联邦聚合器的全局Q值，实现分布式学习。
        通过聚合多个基站的学习经验，提高缓存决策的准确性。
        """
    ...
    def get_reward_popularity(self):
        """
        基于流行度的奖励跟踪方法。
        计算每个内容的流行度得分，用于Popularity-based缓存决策。
        """
    ...
    def cache_cleanup(self, current_time):
        """
        统一的缓存清理方法，适用于所有节点类型（BS、Vehicle、UAV）。
        根据内容有效期清理过期的缓存内容。
        """
        """
    ...
    def get_local_q_value(self, content):
        ...
        参数:
        content (dict): 内容字典，包含类型、坐标、类别和编号等信息。

        返回:
        float: 内容的Q值，如果不存在则返回默认值0.5。
        """
